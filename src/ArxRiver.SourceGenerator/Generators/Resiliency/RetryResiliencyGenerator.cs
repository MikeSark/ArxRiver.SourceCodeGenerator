...
[Generator]
public class RetryResiliencyGenerator : IIncrementalGenerator
{
    private const string TaskManagerName = "RetryResiliency";
                ...
                static (context, token) =>
                    {
                        ...
                        if (symbol is null)
                        {
                            return null;
                                 ...
                                 namedTypeSymbol
                                     .GetAttributes()
                                     .Any(a => a.AttributeClass?.ToDisplayString() == GeneratorConstants.RetryResiliencyAttributeFullPath))
                        {
    ...
    private void CreateOutput(ImmutableArray<TypeSymbolModel> sourceLeft, ImmutableArray<TypeSymbolModel> sourceRight, SourceProductionContext context)
    {
        if (sourceLeft.Length > 0)
        {
            ...
            foreach (var type in sourceLeft.Distinct())
            {
                ...
                var extraName = sourceLeft.Distinct().Count() > 1 ? type.Name : string.Empty;
                if (!sourceRight.Contains(type))
                {
                    RetryResiliencyExecutor.Build(indentWriter, type, accessibleProperties);
                    context.AddSource($"ArxRiver.{extraName}RetryResiliencyExecutor.g.cs", SourceText.From(writer.ToString(), Encoding.UTF8));
                    
                    writer.GetStringBuilder().Clear();

                    ResiliencyDelayGenerators.Build(indentWriter, type, accessibleProperties);
                    context.AddSource($"ArxRiver.{extraName}ResiliencyDelayGenerators.g.cs", SourceText.From(writer.ToString(), Encoding.UTF8));
