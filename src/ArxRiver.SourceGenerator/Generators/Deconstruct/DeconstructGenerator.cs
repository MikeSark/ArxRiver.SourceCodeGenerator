                ...
                static (context, token) =>
                    {
                        ...
                        if (symbol is null)
                        {
                            return null;
                                 ...
                                 namedTypeSymbol
                                     .GetAttributes()
                                     .Any(a => a.AttributeClass?.ToDisplayString() == GeneratorConstants.DeconstructAttributeFullPath))
                        {
        ...
        var excludedTypesHavingExtensionDeconstruct = context.SyntaxProvider
            .CreateSyntaxProvider(
                static (node, _) => node is MethodDeclarationSyntax methodNode &&
                                    ...
                                    methodNode.Identifier.ValueText == DeconstructName &&
                ...
                static (context, token) =>
                    {
                        ...
                        {
                            return new TypeSymbolModel(type,
            ...
            foreach (var type in types.Distinct())
            {
                ...
                if (!excludedTypes.Contains(type))
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(writer.ToString()))
                        context.AddSource($"ArxRiver.Deconstruct.{type.Name.ToSafeClassName()}.g.cs", SourceText.From(writer.ToString(), Encoding.UTF8));
                }
