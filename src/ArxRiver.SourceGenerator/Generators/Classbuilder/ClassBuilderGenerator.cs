...
[Generator]
public class ClassBuilderGenerator : IIncrementalGenerator
{
    private const string ClassBuilderName = "ClassBuilder";
                ...
                static (context, token) =>
                    {
                        ...
                        if (symbol is null)
                        {
                            return null;
                                 ...
                                 namedTypeSymbol
                                     .GetAttributes()
                                     .Any(a => a.AttributeClass?.ToDisplayString() == GeneratorConstants.AutoClassBuilderAttributeFullPath))
                        {
        ...
        var excludedTypesHavingClassbuilders = context.SyntaxProvider
            .CreateSyntaxProvider(
                static (node, _) => node is MethodDeclarationSyntax methodNode &&
                                    ...
                                    methodNode.Identifier.ValueText == ClassBuilderName &&
    ...
    private void CreateOutput(ImmutableArray<TypeSymbolModel> sourceLeft, ImmutableArray<TypeSymbolModel> sourceRight, SourceProductionContext context)
    {
        if (sourceLeft.Length > 0)
        {
            ...
            foreach (var type in sourceLeft.Distinct())
            {
                ...
                {
                    ClassBuilderBuilder.Build(indentWriter, type, accessibleProperties);
                    context.AddSource($"ArxRiver.Builder.{type.Name.ToSafeClassName()}.g.cs", SourceText.From(writer.ToString(), Encoding.UTF8));
